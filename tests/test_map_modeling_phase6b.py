...
import map_modeling_common as mmc                      # noqa: E402
import map_random_forest_tuning_v1 as rf_tune          # noqa: E402
import map_xgboost_tuning_v1 as xgb_tune               # noqa: E402
import preprocessing_common_map_v2 as pcm              # noqa: E402
import preprocessing_random_forest_map_v2 as prep_rf   # noqa: E402
import preprocessing_xgboost_map_v2 as prep_xgb        # noqa: E402
from map_feature_families import feature_family_map, FAMILY_LABELS   # noqa: E402
from map_selected_oof_v1 import ENSEMBLE_WEIGHTS, select_ensemble_weight   # noqa: E402
...
def test_mirroring_is_training_side_only_in_every_harness(roles):
    ...
    src = (SCRIPTS_DIR / "map_modeling_common.py").read_text(encoding="utf-8")
    xgb_src = (MODELS_SCRIPTS_DIR / "map_xgboost_tuning_v1.py").read_text(encoding="utf-8")
    train_src = (SCRIPTS_DIR / "train_map_models_v1.py").read_text(encoding="utf-8")
...
def test_test_partition_is_inaccessible_during_tuning():
    ...
    for name in ["models/map_random_forest_tuning_v1.py", "models/map_xgboost_tuning_v1.py",
...
def test_saved_models_reload_with_identical_predictions(roles):
    ...
    rf = joblib.load(ROOT / "models" / "map_random_forest_v1.joblib")
    p1 = rf.predict_proba(X_rf)[:, 1]
    p2 = joblib.load(ROOT / "models" / "map_random_forest_v1.joblib").predict_proba(X_rf)[:, 1]
    ...
    m1 = XGBClassifier(); m1.load_model(str(ROOT / "models" / "map_xgboost_v1.json"))
    m2 = XGBClassifier(); m2.load_model(str(ROOT / "models" / "map_xgboost_v1.json"))
