...
from feature_engineering.preprocessing.preprocessing_random_forest_map_v3 import EXPECTED_TRANSFORMED_FEATURES  # noqa: E402
from feature_engineering.preprocessing.preprocessing_common_map_v2 import UNKNOWN_MAP_CATEGORY, UNKNOWN_TIER_CATEGORY  # noqa: E402
from training.map_models.map_modeling_common import split_inner_early_stop, N_FOLDS, LOG_LOSS_EQUIVALENCE_EPSILON  # noqa: E402
...
def roles():
    return pcm3.load_map_v3_roles(ROOT / "config" / "features" / "map_features_v3_modern_map.yaml")
...
def test_transformed_width_is_exactly_131(roles):
    from feature_engineering.preprocessing.preprocessing_common_map_v2 import transformed_feature_names
...
def test_unknown_map_and_unknown_tier_supported():
    from feature_engineering.preprocessing.preprocessing_common_map_v2 import transformed_feature_names
    names = transformed_feature_names(pcm3.load_map_v3_roles(ROOT / "config" / "features" / "map_features_v3_modern_map.yaml"))
...
def test_inner_split_chronological_and_atomic_on_real_v3_data():
    from training.map_models.map_modeling_common import load_cv_manifest, fold_frames
...
def test_this_script_never_reads_the_map_split_file():
    for name in ["map_xgboost_v3_final_tuning.py", "finalize_map_xgboost_v3.py"]:
...
def test_final_refit_has_no_eval_set_or_early_stopping():
    src = (ROOT / "training" / "map_models" / "finalize_map_xgboost_v3.py").read_text(encoding="utf-8")
...
def test_no_calibration_artifact_or_import():
    ...
    meta_path = ROOT / "models" / "map" / "map_xgboost_v3_final_metadata.json"
    if meta_path.exists():
...
def test_saved_model_reload_prediction_parity():
    ...
    roles_ = pcm3.load_map_v3_roles(ROOT / "config" / "features" / "map_features_v3_modern_map.yaml")
...
def test_future_composer_inference_parity_synthetic_only():
    ...
    from feature_engineering.maps.rich_modern_map_feature_composer import build_future_modern_rich_map_features
