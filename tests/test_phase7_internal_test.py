...
from feature_engine import elo_expected, ELO_INITIAL          # noqa: E402
from map_modeling_common import baseline_probabilities, series_macro_metrics  # noqa: E402
import phase7_test_bootstrap as boot                            # noqa: E402
import phase7_test_reports as reports                           # noqa: E402
...
def test_protocol_yaml_declares_every_predefined_metric_and_baseline():
    import yaml
    cfg = yaml.safe_load((ROOT / "config" / "phase7_test_evaluation_protocol.yaml").read_text(encoding="utf-8"))
...
def test_protocol_is_frozen_before_the_evaluator_can_execute():
    ...
    src = (SCRIPTS_DIR / "freeze_phase7_protocol.py").read_text(encoding="utf-8")
...
def test_evaluator_contains_no_fit_call():
    src = (SCRIPTS_DIR / "evaluate_phase7_test_once.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
...
def test_no_new_ensemble_construction():
    for name in ["evaluate_phase7_test_once.py", "phase7_test_reports.py"]:
        src = (SCRIPTS_DIR / name).read_text(encoding="utf-8").lower()
...
def test_cologne_access_is_limited_to_group_label_lookup():
    ...
    for name in ["phase7_test_reports.py", "phase7_test_bootstrap.py", "phase7_test_visualizations.py"]:
        s = (SCRIPTS_DIR / name).read_text(encoding="utf-8")
