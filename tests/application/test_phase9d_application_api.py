...
@pytest.fixture(scope="module")
def fixtures():
    return yaml.safe_load((CONFIG / "application" / "application_api_fixtures_v1.yaml").read_text(encoding="utf-8"))
...
def test_cors_config_is_not_wildcard_with_credentials():
    cfg = yaml.safe_load((CONFIG / "application" / "application_api_v1.yaml").read_text(encoding="utf-8"))["cors"]
...
def test_application_api_never_writes_files():
    src = (ROOT / "application" / "api" / "application_api.py").read_text(encoding="utf-8")
