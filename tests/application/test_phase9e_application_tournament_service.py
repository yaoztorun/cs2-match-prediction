...
@pytest.fixture(scope="module")
def fixtures():
    return yaml.safe_load((CONFIG / "application" / "application_tournament_fixtures_v1.yaml").read_text(encoding="utf-8"))
...
def test_tournament_engine_unchanged_hash():
    import hashlib
    actual = hashlib.sha256((ROOT / "tournament" / "engine" / "tournament_engine.py").read_bytes()).hexdigest()
...
def test_router_never_writes_files():
    src = (ROOT / "application" / "tournament" / "application_tournament_router.py").read_text(encoding="utf-8")
