...
def test_registry_never_scans_filesystem_for_latest():
    src = (ROOT / "scripts" / "application_inference.py").read_text(encoding="utf-8")
...
def test_registry_hashes_full_pipeline_not_just_model():
    registry = yaml.safe_load((ROOT / "config" / "application_inference_contexts_v1.yaml")
...
def test_no_map_aliases_invented():
    registry = yaml.safe_load((ROOT / "config" / "application_map_registry_v1.yaml").read_text(encoding="utf-8"))
...
def test_no_model_fitting_imports_in_application_inference():
    tree = ast.parse((ROOT / "scripts" / "application_inference.py").read_text(encoding="utf-8"))
