...
def feature_groups():
    return yaml.safe_load((CONFIG / "application_explanation_feature_groups_v1.yaml").read_text(encoding="utf-8"))
...
def test_phase9b_pytest_suite_passes():
    ...
    result = subprocess.run([sys.executable, "-m", "pytest",
                              "tests/test_phase9b_application_inference.py", "-q"],
...
def test_phase9b_validator_passes():
    ...
    import sys
    import os
    result = subprocess.run([sys.executable, "validate_phase9b.py"], cwd=str(ROOT / "scripts"),
