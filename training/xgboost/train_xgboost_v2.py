...
TRAIN-only three-stage chronological CV in training/xgboost/xgboost_tuning_v2.py) on
...
from feature_engineering.preprocessing.preprocessing_xgboost_v1 import (
...
LR_META = MODELS_DIR / "series" / "logistic_regression_scratch_v1.json"
...
def main():
    ...
    if not SELECTED_CONFIG_PATH.exists():
        raise RuntimeError(f"{SELECTED_CONFIG_PATH} missing - run training/xgboost/xgboost_tuning_v2.py first.")
    ...
    model.save_model(str(MODELS_DIR / "series" / "xgboost_v2.json"))
    ...
    (MODELS_DIR / "series" / "xgboost_v2_metadata.json").write_text(json.dumps(model_meta, indent=2), encoding="utf-8")
...
def write_symmetry(sym_err, sym, xgb1_meta):
    ...
    (REPORTS / "phases" / "xgboost_symmetry_v2.md").write_text("\n".join(md), encoding="utf-8")
...
def write_comparison(lr_meta, rf1_meta, rf2_meta, xgb1_meta, xgb2_train, xgb2_val):
    ...
    (REPORTS / "phases" / "model_comparison_v2.md").write_text("\n".join(md), encoding="utf-8")
...
def write_results(selected, train_raw, val_raw, augmented_train_raw, feature_names,
    ...
    (REPORTS / "phases" / "phase4c1_xgboost_v2_results.md").write_text("\n".join(md), encoding="utf-8")
