...
RF_V1_MODEL_JSON_PATH = MODELS_DIR / "series" / "random_forest_v1.json"
RF_V2_MODEL_JSON_PATH = MODELS_DIR / "series" / "random_forest_v2.json"
...
def main():
    ...
    save_model(model, MODELS_DIR / "series" / "xgboost_v1.json")
    ...
    (MODELS_DIR / "series" / "xgboost_v1_metadata.json").write_text(json.dumps(model_meta, indent=2), encoding="utf-8")
...
def write_symmetry_report(symmetry_error, stats):
    ...
    (REPORTS / "phases" / "xgboost_symmetry_v1.md").write_text("\n".join(md), encoding="utf-8")
...
def write_model_comparison(lr_meta, rf1_meta, rf2_meta, xgb_train, xgb_val):
    ...
    (REPORTS / "phases" / "model_comparison_v1.md").write_text("\n".join(md), encoding="utf-8")
...
def write_phase4c_report(train_raw, val_raw, augmented_train_raw, feature_names, params,
    ...
    (REPORTS / "phases" / "phase4c_xgboost_v1.md").write_text("\n".join(md), encoding="utf-8")
