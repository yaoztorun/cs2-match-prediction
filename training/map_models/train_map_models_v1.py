...
def main():
    ...
    joblib.dump(rf, MODELS_DIR / "map" / "map_random_forest_v1.joblib")
    ...
    xgb.save_model(str(MODELS_DIR / "map" / "map_xgboost_v1.json"))

    # ---------------- save/load prediction parity ----------------
    rf_reloaded = joblib.load(MODELS_DIR / "map" / "map_random_forest_v1.joblib")
    xgb_reloaded = XGBClassifier()
    xgb_reloaded.load_model(str(MODELS_DIR / "map" / "map_xgboost_v1.json"))
    ...
    common_meta = {
        ...
        "feature_config": "config/features/map_features_v2_rich.yaml",
    ...
    (MODELS_DIR / "map" / "map_random_forest_v1_metadata.json").write_text(json.dumps({
    ...
    (MODELS_DIR / "map" / "map_xgboost_v1_metadata.json").write_text(json.dumps({
...
def write_report(rf_sel, xgb_sel, ens_cfg, val_metrics, series_macro, per_map, coverage, calib_tables,
    ...
    (REPORTS / "phases" / "phase6b_known_map_model_results.md").write_text("\n".join(md), encoding="utf-8")
