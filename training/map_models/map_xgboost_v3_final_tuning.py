...
training/map_models/finalize_map_xgboost_v3.py. A0 here answers "does this structure
...
from feature_engineering.preprocessing.preprocessing_common_map_v3 import load_map_v3_roles, EXPECTED_RAW_PREDICTIVE_INPUTS
from feature_engineering.preprocessing.preprocessing_common_map_v2 import build_augmented_training_raw, assert_augmented_symmetry
...
def assert_feature_freeze(roles):
    ...
    from training.map_models.map_modeling_common import FORBIDDEN_PREDICTORS
...
class XgbFoldCache:
    """Mirrors training/map_models/map_xgboost_tuning_v1.py's own XgbFoldCache
...
def build_search_plan_v3(candidates):
    plan = {
        ...
        "artifact_hashes": {
            "config/features/map_features_v3_modern_map.yaml": sha256_file(CONFIG_PATH),
...
def write_report(agg, winner_id, stage, params_by_id, wagg, best_iterations, final_n_estimators,
    ...
    md.append(f"Generated by `training/map_models/map_xgboost_v3_final_tuning.py`. **{len(agg)} candidates** "
    ...
    md.append("`A0_phase6b_reference_structure` carries the Phase 6B structural hyperparameters, but its "
              ...
              "conflated in this report or in `training/map_models/finalize_map_xgboost_v3.py`'s three-way comparison.\n")
    ...
    (REPORTS / "phases" / "phase6d_final_xgboost_v3_tuning.md").write_text("\n".join(md), encoding="utf-8")
