...
from feature_engineering.preprocessing.preprocessing_common_map_v2 import EXPECTED_TRANSFORMED_FEATURES  # noqa: E402
...
def write_report(agg, winner_id, stage, params_by_id, wagg):
    ...
    md.append(f"Generated by `training/map_models/map_random_forest_tuning_v1.py`. **{len(agg)} candidates** "
    ...
    md.append("This script never opens `data/modeling/map_split_v1.csv`. It reads only the Phase 6A CV manifest, "
              ...
              "`validation/validate_phase6b.py`). The 1,129-map main validation partition and the 1,427-map TEST "
    ...
    (REPORTS / "phases" / "phase6b_map_random_forest_tuning.md").write_text("\n".join(md), encoding="utf-8")
