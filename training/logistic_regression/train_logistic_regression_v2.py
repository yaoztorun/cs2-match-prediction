...
training/logistic_regression/logistic_regression_tuning_v2.py) on the FULL augmented TRAIN
...
implementation (training/logistic_regression/logistic_regression_scratch.py) - only the
...
from feature_engineering.preprocessing.preprocessing_logistic_v1 import (
...
LR1_META = MODELS_DIR / "series" / "logistic_regression_scratch_v1.json"
RF1_META = MODELS_DIR / "series" / "random_forest_v1.json"
RF2_META = MODELS_DIR / "series" / "random_forest_v2.json"
XGB1_META = MODELS_DIR / "series" / "xgboost_v1_metadata.json"
XGB2_META = MODELS_DIR / "series" / "xgboost_v2_metadata.json"
...
def main():
    ...
    if not SELECTED_CONFIG_PATH.exists():
        raise RuntimeError(f"{SELECTED_CONFIG_PATH} missing - run training/logistic_regression/logistic_regression_tuning_v2.py first.")
    ...
    np.savez(MODELS_DIR / "series" / "logistic_regression_scratch_v2.npz",
    ...
    model_meta = {
        ...
        "feature_config_path": "config/features/series_features_v1.yaml",
    ...
    (MODELS_DIR / "series" / "logistic_regression_scratch_v2.json").write_text(
...
def write_symmetry(sym_err, sym):
    ...
    (REPORTS / "phases" / "logistic_regression_symmetry_v2.md").write_text("\n".join(md), encoding="utf-8")
...
def write_comparison(lr1, rf1, rf2, xgb1, xgb2, lr2_train, lr2_val):
    ...
    (REPORTS / "phases" / "model_comparison_tuned_v1features.md").write_text("\n".join(md), encoding="utf-8")
...
def write_results(selected, train_raw, val_raw, augmented, feature_names, info,
    ...
    (REPORTS / "phases" / "phase4a1_logistic_regression_v2_results.md").write_text("\n".join(md), encoding="utf-8")
