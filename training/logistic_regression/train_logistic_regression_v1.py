...
def main():
    ...
    np.savez(
        MODELS_DIR / "series" / "logistic_regression_scratch_v1.npz",
    ...
    (MODELS_DIR / "series" / "logistic_regression_scratch_v1.json").write_text(
...
def write_report(train_raw, val_raw, augmented_train_raw, feature_names, params,
    ...
    md.append("The model is implemented entirely from scratch in `training/logistic_regression/logistic_regression_scratch.py`, "
    ...
    md.append("Clearly marked `[PROJECT ADAPTATION]`/`[PROJECT ADDITION]` in the source: numerical-stability "
              ...
              "0.5); the entire `feature_engineering/preprocessing/preprocessing_logistic_v1.py` module (imputation/standardization/"
              "reference encoding, mirrored augmentation); model serialization to `.npz`/`.json`.\n")

    md.append("## 5. Chronological split methodology\n")
    md.append(f"`feature_engineering/series/build_series_split_v1.py` splits the 9,456-row Phase 3 development set by exact "
    ...
    md.append("Phase 1/2.5 established a persistent ~55% Team1 win-rate artifact from how `team1`/`team2` are "
              ...
              "(`tests/features/test_preprocessing_logistic.py`) proves the *same fitted artifact* transforms a genuinely "
    ...
    (REPORTS / "phases" / "phase4a_logistic_regression_v1.md").write_text("\n".join(md), encoding="utf-8")
