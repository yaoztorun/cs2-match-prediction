...
train-only chronological CV in training/random_forest/random_forest_tuning_v2.py) on the
...
def main():
    ...
    if not SELECTED_CONFIG_PATH.exists():
        raise RuntimeError(f"{SELECTED_CONFIG_PATH} does not exist - run training/random_forest/random_forest_tuning_v2.py first.")
    ...
    import joblib
    joblib.dump(model, MODELS_DIR / "series" / "random_forest_v2.joblib")
    ...
    (MODELS_DIR / "series" / "random_forest_v2.json").write_text(json.dumps(model_meta, indent=2), encoding="utf-8")
...
def write_final_report(train_raw, val_raw, augmented_train_raw, feature_names, params, rf_params, selected,
    ...
    (REPORTS / "phases" / "phase4b1_random_forest_v2_results.md").write_text("\n".join(md), encoding="utf-8")
