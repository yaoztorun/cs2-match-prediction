...
from application.tournament.application_tournament_router import router as major_router
...
_API_CONFIG = yaml.safe_load((CONFIG / "application" / "application_api_v1.yaml").read_text(encoding="utf-8"))
...
def _verify_prediction_and_explanation_contract():
    ...
    explanation_ok = True
    try:
        registry = yaml.safe_load((CONFIG / "application" / "application_inference_contexts_v1.yaml").read_text(encoding="utf-8"))
    ...
    try:
        ...
        explanation_ok = (
            exp_hashes["application_explanations_py"] == bar.sha256_file(ROOT / "application" / "explanations" / "application_explanations.py")
            and exp_hashes["application_inference_py"] == bar.sha256_file(ROOT / "application" / "inference" / "application_inference.py")
            and exp_hashes["feature_groups_registry"] == bar.sha256_file(
                CONFIG / "application" / "application_explanation_feature_groups_v1.yaml")
            and exp_hashes["phase9b_context_registry"] == bar.sha256_file(
                CONFIG / "application" / "application_inference_contexts_v1.yaml")
