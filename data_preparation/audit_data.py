...
def main():
    ...
    dd.to_csv(REPORTS / "phases" / "data_dictionary.csv", index=False, encoding="utf-8")

    # ---------------- data_audit.md ----------------
    md = []
    md.append("# CS2 Match Prediction — Raw Data Audit\n")
    md.append("Generated by `data_preparation/audit_data.py` from the files under `data/raw/`. "
    ...
    (REPORTS / "phases" / "data_audit.md").write_text("\n".join(md), encoding="utf-8")

    # ---------------- leakage_analysis.md ----------------
    lk = []
    lk.append("# Leakage Analysis\n")
    lk.append("Generated by `data_preparation/audit_data.py`. Covers (1) why the provided `team1_win` column cannot be used as the label, "
    ...
    (REPORTS / "phases" / "leakage_analysis.md").write_text("\n".join(lk), encoding="utf-8")
