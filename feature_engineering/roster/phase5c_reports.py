...
DISCIPLINE FOR THE QUALITY REPORT
  * computed on the GLOBAL TRAIN partition only (data/modeling/series_split_v1.csv),
    the same discipline feature_engineering/maps/phase5a_reports.py and phase5b2_reports.py use;
...
from feature_engineering.roster.player_roster_stream_common import MIN_ROUNDS_FOR_VALID_MAP
...
def write_engineering_report(summary):
    ...
    md.append("`feature_engineering/state/build_pre_cologne_player_roster_state_v1.py` replays the canonical stream "
    ...
    (REPORTS / "phases" / "phase5c_player_roster_feature_engineering.md").write_text("\n".join(md), encoding="utf-8")
...
def write_quality_report(summary):
    ...
    md.append("**Scope discipline.** Every number below is computed on the **global TRAIN "
              ...
              "`feature_engineering/maps/phase5a_reports.py` and `phase5b2_reports.py` established. Validation is not "
    ...
    md.append(f"Exactly the {len(ROSTER_PERFORMANCE_DIFFS)} performance diffs carry NaN, and only "
              ...
              "at build time and re-checked by `validation/validate_phase5c.py`. The remaining "
    ...
    (REPORTS / "phases" / "phase5c_player_roster_feature_quality.md").write_text("\n".join(md), encoding="utf-8")
