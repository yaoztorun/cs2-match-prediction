...
Random Forest preprocessing for the KNOWN-MAP feature set. Same semantics as
the frozen series RF preprocessing modules - median imputation of continuous
features, NO scaling (tree splits are scale-invariant), deterministic
reference-dummy categorical encoding - extended to the map task's `map_name`
one-hot with its explicit `__UNKNOWN_MAP__` / `__UNKNOWN_TIER__` categories.

Leakage safety: medians are computed from the AUGMENTED FOLD-TRAIN (or full
augmented TRAIN) rows the caller hands in and from nothing else. Never global,
never validation, never test.
"""
