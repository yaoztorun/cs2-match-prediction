...
def write_engineering_report(mapf, ser, v1, snap, map_sum, ser_sum, snap_meta):
    with open(ROOT / "config" / "features" / "map_features_v1.yaml", encoding="utf-8") as f:
        map_cfg = yaml.safe_load(f)
    with open(ROOT / "config" / "features" / "series_features_v2_map_pool.yaml", encoding="utf-8") as f:
    ...
    A("## 15. Test coverage\n")
    A("`tests/features/test_map_feature_engine.py` - 73 synthetic-fixture tests, no dependency on the real "
    ...
    (REPORTS / "phases" / "phase5a_map_feature_engineering.md").write_text("\n".join(L), encoding="utf-8")
...
def write_quality_report(mapf, ser, split, snap, ser_sum):
    ...
    (REPORTS / "phases" / "phase5a_map_feature_quality.md").write_text("\n".join(L), encoding="utf-8")
