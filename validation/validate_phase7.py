...
PHASE7_SCRIPT_DIRS = {"evaluate_phase7_test_once.py": ROOT / "evaluation" / "internal_test",
...
FROZEN_PHASE1_6D_ARTIFACTS = [
    "models/map/map_xgboost_v3_final.json", "models/map/map_xgboost_v3_final_metadata.json",
    "data/modeling/map_xgboost_v3_final_preprocessing.json", "data/modeling/map_xgboost_v3_final_config.json",
    "config/features/map_features_v3_modern_map.yaml", "data/features/map_features_v3_modern_map.parquet",
    ...
    "reports/phases/phase6d_final_xgboost_v3_results.md",
]
...
def main():
    ...
    reader_path = PHASE7_SCRIPT_DIRS[PHASE7_SOLE_TEST_READER] / PHASE7_SOLE_TEST_READER
    ...
    for name in PHASE7_DOWNSTREAM_SCRIPTS:
        src = (PHASE7_SCRIPT_DIRS[name] / name).read_text(encoding="utf-8")
    ...
    for r in ["phase7_internal_test_results.md", "phase7_internal_test_uncertainty.md"]:
        check(f"report exists: {r}", (ROOT / "reports" / "phases" / r).exists())
    ...
    if ci_path.exists():
        ...
        check("bootstrap CI table has all four metrics", set(ci_df["metric"]) == {"accuracy", "roc_auc",
                                                                                     "log_loss", "brier"})
    boot_src = (ROOT / "evaluation" / "uncertainty" / "phase7_test_bootstrap.py").read_text(encoding="utf-8")
