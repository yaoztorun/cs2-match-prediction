...
def main():
    ...
    fg = yaml.safe_load((ROOT / "config" / "application" / "application_explanation_feature_groups_v1.yaml")
    ...
    ae_path = ROOT / "application" / "explanations" / "application_explanations.py"
    ...
    result = subprocess.run([sys.executable, "-m", "pytest", "tests/application/test_phase9b_application_inference.py", "-q"],
    ...
    result2 = subprocess.run([sys.executable, "-m", "validation.validate_phase9b"], cwd=str(ROOT),
                              capture_output=True, text=True, encoding="utf-8", env=env)
    check("validation/validate_phase9b.py passes (real subprocess run)", result2.returncode == 0)
    if result2.returncode != 0:
        print(result2.stdout[-2000:])

    print("\n=== 9. fixture manifest / receipt ===")
    fixture_manifest_path = ROOT / "config" / "application" / "application_explanation_fixtures_v1.yaml"
    ...
    report_path = ROOT / "reports" / "phases" / "phase9c_application_explanation_core.md"
