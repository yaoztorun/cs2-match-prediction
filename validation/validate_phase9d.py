...
def main():
    api_py = ROOT / "application" / "api" / "application_api.py"
    ...
    if receipt_path.exists():
        ...
        h = receipt["hashes"]
        fresh = {
            "application_api_py": sha256_file(ROOT / "application" / "api" / "application_api.py"),
            "run_application_api_py": sha256_file(ROOT / "application" / "api" / "run_application_api.py"),
            "api_config": sha256_file(CONFIG / "application" / "application_api_v1.yaml"),
            "fixture_manifest": sha256_file(CONFIG / "application" / "application_api_fixtures_v1.yaml"),
            ...
            "application_inference_py": sha256_file(ROOT / "application" / "inference" / "application_inference.py"),
            "application_explanations_py": sha256_file(ROOT / "application" / "explanations" / "application_explanations.py"),
            ...
            "test_phase9d_application_api_py": sha256_file(TESTS / "application" / "test_phase9d_application_api.py"),
        }
    ...
    with TestClient(api.app) as client:
        ...
        fixtures = yaml.safe_load((CONFIG / "application" / "application_api_fixtures_v1.yaml").read_text(encoding="utf-8"))
    ...
    import os
    env = {**os.environ, **env}
    r1 = subprocess.run([sys.executable, "-m", "pytest", "tests/application/test_phase9b_application_inference.py", "-q"],
    ...
    r2 = subprocess.run([sys.executable, "-m", "pytest", "tests/application/test_phase9c_application_explanations.py", "-q"],
                         cwd=str(ROOT), capture_output=True, text=True, encoding="utf-8", env=env)
    check("full Phase 9C pytest suite still passes", r2.returncode == 0)
    r3 = subprocess.run([sys.executable, "-m", "validation.validate_phase9b"], cwd=str(ROOT), capture_output=True, text=True,
                         encoding="utf-8", env=env)
    check("validation/validate_phase9b.py still passes", r3.returncode == 0)
    r4 = subprocess.run([sys.executable, "-m", "validation.validate_phase9c"], cwd=str(ROOT), capture_output=True, text=True,
                         encoding="utf-8", env=env)
    check("validation/validate_phase9c.py still passes", r4.returncode == 0)
    for label, res in (("9B pytest", r1), ("9C pytest", r2), ("validate_phase9b", r3), ("validate_phase9c", r4)):
        if res.returncode != 0:
            print(f"  --- {label} tail ---\n{res.stdout[-2000:]}")

    print("\n=== 15. report exists with required markers ===")
    report_path = ROOT / "reports" / "phases" / "phase9d_application_api.md"
