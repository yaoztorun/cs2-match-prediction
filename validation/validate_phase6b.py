...
import training.map_models.map_modeling_common as mmc
import training.map_models.map_random_forest_tuning_v1 as rf_tune
import training.map_models.map_xgboost_tuning_v1 as xgb_tune
import feature_engineering.preprocessing.preprocessing_common_map_v2 as pcm
import feature_engineering.preprocessing.preprocessing_random_forest_map_v2 as prep_rf
import feature_engineering.preprocessing.preprocessing_xgboost_map_v2 as prep_xgb
from training.map_models.map_selected_oof_v1 import ENSEMBLE_WEIGHTS, select_ensemble_weight
...
TUNING_SCRIPTS = [
    ROOT / "training" / "map_models" / "map_random_forest_tuning_v1.py",
    ROOT / "training" / "map_models" / "map_xgboost_tuning_v1.py",
    ROOT / "training" / "map_models" / "map_selected_oof_v1.py",
    ROOT / "training" / "map_models" / "map_baselines_v1.py",
]

# Everything Phase 1-6A produced that Phase 6B must not have touched.
FROZEN_PATHS = [
    "feature_engineering/series/feature_engine.py", "feature_engineering/maps/map_feature_engine.py", "feature_engineering/form/team_form_engine.py",
    "feature_engineering/roster/player_roster_feature_engine.py", "feature_engineering/maps/rich_map_feature_composer.py",
    "feature_engineering/maps/build_map_features_v2_rich.py", "feature_engineering/maps/build_map_split_v1.py",
    "feature_engineering/maps/build_map_cv_folds_v1.py", "validation/validate_phase6a.py",
    "feature_engineering/preprocessing/preprocessing_common.py", "feature_engineering/preprocessing/preprocessing_common_v2_map_pool.py",
    "feature_engineering/preprocessing/preprocessing_common_v3_form.py", "feature_engineering/preprocessing/preprocessing_common_v4_roster.py",
    "feature_engineering/preprocessing/preprocessing_random_forest_v1.py", "feature_engineering/preprocessing/preprocessing_random_forest_v2_map_pool.py",
    "feature_engineering/preprocessing/preprocessing_xgboost_v1.py", "feature_engineering/preprocessing/preprocessing_xgboost_v2_map_pool.py",
    "training/random_forest/random_forest_tuning_v2.py", "training/xgboost/xgboost_tuning_v2.py",
    ...
    "config/features/series_features_v1.yaml", "config/features/series_features_v4_roster.yaml",
    ...
    "models/series/random_forest_v2.joblib", "models/series/xgboost_v2.json",
    "models/series/logistic_regression_scratch_v2.npz",
    "reports/phases/phase6a_map_v2_feature_quality.md",
]
...
def main():
    ...
    for d in ["feature_engineering", "training", "training/map_models", "evaluation", "tournament", "application",
              "validation", "tests", "data", "config", "reports", "models"]:
    ...
    print("\n=== 9. full-TRAIN refits used the frozen configurations ===")
    rf_meta = json.loads((MODELS / "map" / "map_random_forest_v1_metadata.json").read_text(encoding="utf-8"))
    xgb_meta = json.loads((MODELS / "map" / "map_xgboost_v1_metadata.json").read_text(encoding="utf-8"))
    ...
    rf_a = joblib.load(MODELS / "map" / "map_random_forest_v1.joblib")
    rf_b = joblib.load(MODELS / "map" / "map_random_forest_v1.joblib")
    p_rf_a, p_rf_b = rf_a.predict_proba(X_rf)[:, 1], rf_b.predict_proba(X_rf)[:, 1]
    xg_a, xg_b = XGBClassifier(), XGBClassifier()
    xg_a.load_model(str(MODELS / "map" / "map_xgboost_v1.json"))
    xg_b.load_model(str(MODELS / "map" / "map_xgboost_v1.json"))
    ...
    results_md = (REPORTS_DIR / "phases" / "phase6b_known_map_model_results.md").read_text(encoding="utf-8")
