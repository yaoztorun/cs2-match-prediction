...
combination, so scripts/validate_phase5b3.py requires the V2-arm fold metrics
...
Writes:
    ...
    data/modeling/phase5b3_fold_preprocessing_audit.json   (for scripts/validate_phase5b3.py)
"""
...
def write_report(fold_df, agg_df, delta_df, delta_agg, rf_importance_df, xgb_importance_df, group_dfs,
    ...
    md.append(f"**Regression parity.** V2-arm fold metrics here are required (by "
              "`scripts/validate_phase5b3.py`) to match Phase 5B.1's own saved V2-arm rows in "
    ...
    (REPORTS / "phase5b3_team_form_cv_results.md").write_text("\n".join(md), encoding="utf-8")
