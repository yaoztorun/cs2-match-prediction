...
from preprocessing_xgboost_v1 import build_augmented_training_raw, fit_preprocessing, transform
...
def write_report(fold_df, agg_df, winner_id, stage, params_by_id, best_iterations, final_n_estimators):
    ...
    md.append(f"Generated by `scripts/xgboost_tuning_v2.py`. **{len(agg_df)} candidates** "
    ...
    md.append("This script never opens `data/modeling/series_split_v1.csv`; it reads only the RF V2 fold "
              ...
              "`scripts/validate_phase4c1.py`). The 1,419-match main validation partition therefore cannot have "
    ...
    md.append("This configuration is now frozen in `data/modeling/xgboost_v2_selected_config.json`. Only after "
              ...
              "`scripts/train_xgboost_v2.py`.\n")

    (REPORTS / "phase4c1_xgboost_tuning.md").write_text("\n".join(md), encoding="utf-8")
