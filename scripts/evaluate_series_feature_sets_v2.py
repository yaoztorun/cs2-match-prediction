...
Writes:
    ...
    data/modeling/phase5b1_fold_preprocessing_audit.json   (for scripts/validate_phase5b1.py)
"""
...
def main():
    ...
    if not RF_SELECTED_CONFIG_PATH.exists():
        raise RuntimeError(f"{RF_SELECTED_CONFIG_PATH} missing - run scripts/random_forest_tuning_v2.py first.")
    if not XGB_SELECTED_CONFIG_PATH.exists():
        raise RuntimeError(f"{XGB_SELECTED_CONFIG_PATH} missing - run scripts/xgboost_tuning_v2.py first.")
    ...
    for fold in range(1, N_FOLDS + 1):
        for fs_name, fs in feature_sets.items():
            ...
            for model_name, fit_fn, transform_fn, build_model_fn in [
                ...
                # preprocessing audit entry (for scripts/validate_phase5b1.py's
...
def write_report(fold_df, agg_df, delta_df, delta_agg, rf_importance_df, xgb_importance_df,
    ...
    (REPORTS / "phase5b1_series_map_pool_cv_results.md").write_text("\n".join(md), encoding="utf-8")
