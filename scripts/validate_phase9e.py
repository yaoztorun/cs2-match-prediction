...
def main():
    ...
    if receipt_path.exists():
        ...
        h = receipt["hashes"]
        fresh = {
            "application_tournament_service_py": sha256_file(SCRIPTS / "application_tournament_service.py"),
            "application_tournament_router_py": sha256_file(SCRIPTS / "application_tournament_router.py"),
            "ruleset_registry": sha256_file(CONFIG / "application_tournament_rulesets_v1.yaml"),
            "fixture_manifest": sha256_file(CONFIG / "application_tournament_fixtures_v1.yaml"),
            "tournament_engine_py": sha256_file(SCRIPTS / "tournament_engine.py"),
            ...
            "test_phase9e_py": sha256_file(TESTS / "test_phase9e_application_tournament_service.py"),
        }
    ...
    check("tournament_engine.py hash == Phase 8D-recorded value",
          sha256_file(SCRIPTS / "tournament_engine.py") == "012bd58e7792f7cce1e888d8f233fab274d798d4c3728f5b237f041fb73dd665")
    ...
    hits = module_reads_forbidden(SCRIPTS / "application_tournament_service.py",
    ...
    src = (SCRIPTS / "application_tournament_service.py").read_text(encoding="utf-8")
    ...
    r1 = subprocess.run([sys.executable, "-m", "pytest", "tests/test_phase8c_tournament_engine.py", "-q"],
    ...
    r2 = subprocess.run([sys.executable, "validate_phase8c.py"], cwd=str(SCRIPTS), capture_output=True, text=True,
                         encoding="utf-8", env=env)
    check("scripts/validate_phase8c.py still passes", r2.returncode == 0)
    r3 = subprocess.run([sys.executable, "-m", "pytest", "tests/test_phase9d_application_api.py", "-q"],
                         cwd=str(ROOT), capture_output=True, text=True, encoding="utf-8", env=env)
    check("full Phase 9D pytest suite still passes (transport-identical)", r3.returncode == 0)
    r4 = subprocess.run([sys.executable, "validate_phase9b.py"], cwd=str(SCRIPTS), capture_output=True, text=True,
                         encoding="utf-8", env=env)
    check("scripts/validate_phase9b.py still passes", r4.returncode == 0)
    r5 = subprocess.run([sys.executable, "validate_phase9c.py"], cwd=str(SCRIPTS), capture_output=True, text=True,
                         encoding="utf-8", env=env)
    check("scripts/validate_phase9c.py still passes", r5.returncode == 0)
    r6 = subprocess.run([sys.executable, "validate_phase9d.py"], cwd=str(SCRIPTS), capture_output=True, text=True,
                         encoding="utf-8", env=env)
    check("scripts/validate_phase9d.py still passes", r6.returncode == 0)
    ...
    report_path = ROOT / "reports" / "phase9e_application_tournament_service.md"
