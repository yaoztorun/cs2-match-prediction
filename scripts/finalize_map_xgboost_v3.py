...
Three responsibilities, run only after scripts/models/map_xgboost_v3_final_tuning.py
has frozen a structural configuration and final_n_estimators:
  ...
  3. FULL-TRAIN REFIT: the full-TRAIN map universe is explicitly
     ...
     against scripts/rich_modern_map_feature_composer.py using a SYNTHETIC
...
sys.path.insert(0, str(Path(__file__).resolve() / ".." / "models"))
...
from preprocessing_common_map_v3 import load_map_v3_roles
from preprocessing_common_map_v2 import build_augmented_training_raw, assert_augmented_symmetry, mirror_raw_rows
...
from rich_modern_map_feature_composer import (
...
from evaluate_map_feature_sets_v3 import V3_FAMILIES, V3_FAMILY_LABELS, grouped_permutation_importance

sys.path.insert(0, str(ROOT / "scripts" / "models"))
from map_xgboost_v3_final_tuning import XgbFoldCache   # noqa: E402
...
from map_feature_families import _RAW_FAMILIES as V2_RAW_FAMILIES, FAMILY_LABELS as V2_FAMILY_LABELS
...
def main():
    ...
    final_model.save_model(str(MODELS_DIR / "map_xgboost_v3_final.json"))
    prep_xgb.save_preprocessing(final_prep, MODELING_DIR / "map_xgboost_v3_final_preprocessing.json")

    # ---- reload parity ----
    reloaded = XGBClassifier()
    reloaded.load_model(str(MODELS_DIR / "map_xgboost_v3_final.json"))
    ...
    (MODELS_DIR / "map_xgboost_v3_final_metadata.json").write_text(
...
def write_report(selected, pooled_m, pooled_sm, symmetry_stats, arm_a, arm_b, arm_c, feature_gain, tuning_gain,
    ...
    (REPORTS / "phase6d_final_xgboost_v3_results.md").write_text("\n".join(md), encoding="utf-8")
