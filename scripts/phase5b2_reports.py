...
DISCIPLINE FOR THE QUALITY REPORT
  * computed on the GLOBAL TRAIN partition only (data/modeling/series_split_v1.csv),
    the same discipline scripts/phase5a_reports.py already established;
...
from team_form_engine import FORM_DIRECTIONAL_FEATURES, FORM_SYMMETRIC_FEATURES, FORM_HALF_LIFE_DAYS
...
def write_engineering_report():
    ...
    md.append("`scripts/feature_engine.py` (frozen, never modified) computes `elo_expected`/`elo_update` as "
              ...
              "`scripts/team_form_engine.py` therefore replays the SAME series stream (same rows, same "
    ...
    md.append("**This is not merely asserted to reproduce Phase 3 - it is exhaustively verified.** Before "
              "`series_features_v3_form.parquet` is written, `scripts/build_series_features_v3_form.py` "
              ...
              "Re-verified independently in `scripts/validate_phase5b2.py` from the audit parquet.\n")
    ...
    md.append("`expected_win_prob = elo_expected(own_elo_before, opponent_elo_before)` is always computed "
              ...
              "Directly tested for all four scenarios in `tests/test_team_form_engine.py`.\n")
    ...
    md.append("`normalized_series_margin(score_for, score_against) = (score_for - score_against) / "
              ...
              "convention `scripts/map_feature_engine.py` uses for map margins (a local copy, not an import, "
    ...
    md.append("`process_form_stream` uses the identical two-phase per-timestamp-group protocol as "
              ...
              "`scripts/validate_phase5b2.py` (a real shared-timestamp group is rebuilt from a pre-group state "
              "snapshot and every match in the group is confirmed to see that snapshot, not any other match's "
              "result from the same group) and on synthetic fixtures in `tests/test_team_form_engine.py`.\n")
    ...
    (REPORTS / "phase5b2_team_form_feature_engineering.md").write_text("\n".join(md), encoding="utf-8")
...
def write_quality_report():
    ...
    md.append("**Scope discipline.** Every number below is computed on the **global TRAIN partition** "
              "(`data/modeling/series_split_v1.csv`), the same discipline `scripts/phase5a_reports.py` "
    ...
    (REPORTS / "phase5b2_team_form_feature_quality.md").write_text("\n".join(md), encoding="utf-8")
