...
import build_deployment_history_manifest as bhm
import build_deployment_series_state as bss
import build_deployment_map_state as bms
import build_deployment_form_state as bfs
import build_deployment_roster_state as brs
import build_deployment_modern_map_state as bmms
...
def main():
    ...
    print("\n=== running scripts/validate_phase9a.py before committing the receipt ===")
    import subprocess
    import sys
    import os
    result = subprocess.run([sys.executable, str(ROOT / "scripts" / "validate_phase9a.py")],
                             cwd=str(ROOT / "scripts"), capture_output=True, text=True, encoding="utf-8",
                             env={"PYTHONIOENCODING": "utf-8", **os.environ})
    print(result.stdout[-3000:])
    if result.returncode != 0:
        raise RuntimeError(f"STOP: scripts/validate_phase9a.py failed - refusing to write the receipt. "
                            f"stderr: {result.stderr[-2000:]}")

    import build_deployment_state_receipt as receipt_mod
