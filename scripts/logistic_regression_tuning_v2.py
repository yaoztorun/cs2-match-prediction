...
lambda selection (AST-verified in scripts/validate_phase4a1.py).
...
def write_report(fold_df, agg_df, safety_df, selected_lambda, stage):
    ...
    md.append("Frozen in `data/modeling/logistic_regression_v2_selected_config.json`. Only now may the main "
              "validation partition be evaluated, exactly once, in `scripts/train_logistic_regression_v2.py`.\n")

    (REPORTS / "phase4a1_logistic_regression_tuning.md").write_text("\n".join(md), encoding="utf-8")
