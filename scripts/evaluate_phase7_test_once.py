...
scripts/validate_phase7.py enforces this as an executable AST check, not just
a convention.

Contains NO model-fitting call anywhere (also AST-checked by the validator
and by tests/test_phase7_internal_test.py).
...
from preprocessing_common_map_v2 import mirror_raw_rows
import preprocessing_xgboost_map_v3 as prep_xgb
from map_modeling_common import baseline_probabilities
...
MODEL_META_PATH = ROOT / "models" / "map_xgboost_v3_final_metadata.json"
...
def main():
    if not PROTOCOL_PATH.exists():
        raise RuntimeError(f"{PROTOCOL_PATH} does not exist - run scripts/freeze_phase7_protocol.py first.")
