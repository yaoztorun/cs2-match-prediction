...
from models.random_forest_v1 import build_model, save_model, RF_CONFIG
...
def main():
    ...
    save_model(model, MODELS_DIR / "random_forest_v1.joblib")
    ...
    (MODELS_DIR / "random_forest_v1.json").write_text(json.dumps(model_meta, indent=2), encoding="utf-8")
...
def write_symmetry_report(symmetry_error, symmetry_stats):
    ...
    (REPORTS / "random_forest_symmetry_v1.md").write_text("\n".join(md), encoding="utf-8")
...
def write_comparison_report(lr_meta, rf_val_metrics, rf_train_metrics):
    ...
    (REPORTS / "model_comparison_lr_vs_rf_v1.md").write_text("\n".join(md), encoding="utf-8")
...
def write_phase4b_report(train_raw, val_raw, augmented_train_raw, feature_names, params,
    ...
    md.append(f"Identical raw-mirroring policy to Logistic Regression (shared implementation in "
              f"`scripts/preprocessing_common.py`): {len(train_raw):,} unique historical training matches are "
    ...
    (REPORTS / "phase4b_random_forest_v1.md").write_text("\n".join(md), encoding="utf-8")
