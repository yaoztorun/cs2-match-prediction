...
import map_feature_engine as mfe
import team_form_engine as tfe
import player_roster_feature_engine as prfe
import modern_map_feature_engine as mmfe
...
CONFIG = ROOT / "config"
DATA = ROOT / "data"
CONTEXTS_REGISTRY_PATH = CONFIG / "application_inference_contexts_v1.yaml"
MAP_REGISTRY_PATH = CONFIG / "application_map_registry_v1.yaml"
IDENTITY_POLICY_PATH = DATA / "interim" / "team_identity_policy.csv"
XGB_ROLES_CONFIG_PATH = CONFIG / "map_features_v3_modern_map.yaml"
...
def _load_shared_xgb():
    if "model" not in _SHARED_XGB:
        model = XGBClassifier()
        model.load_model(str(ROOT / "models" / "map_xgboost_v3_final.json"))
