...
from preprocessing_random_forest_v1 import build_augmented_training_raw, fit_preprocessing, transform
...
def main():
    ...
    # construction (scripts/random_forest_cv_folds_v2.py) contains only
    # TRAIN match_ids.
...
def write_report(agg_df, is_v1_ref, winner_id, selection_stage, params_by_id):
    ...
    md.append(f"Generated by `scripts/random_forest_tuning_v2.py`. **{len(agg_df)} candidates** evaluated "
    ...
    md.append("This configuration is now **frozen** (saved to "
              ...
              "`scripts/train_random_forest_v2.py` - no further tuning iteration follows.\n")

    (REPORTS / "phase4b1_random_forest_tuning.md").write_text("\n".join(md), encoding="utf-8")
