...
scripts/modern_map_stream_common.py) rather than a locally mutated ELO, so
...
NO MAP_SLOT
-----------
The map-order audit (see scripts/build_map_features_v3_modern_map.py) found
...
from feature_engine import ELO_INITIAL, elo_expected
...
from player_roster_feature_engine import infer_expected_roster, compute_player_form
...
def apply_selected_map_team_result(store, row):
    ...
    already joined in by scripts/modern_map_stream_common.py from the frozen
...
def apply_selected_map_player_observation(store, row):
    ...
    carry `map_name` (joined in by scripts/modern_map_stream_common.py) in
...
def process_modern_map_stream(map_state, modern_state, player_roster_state, map_rows, emit_features=True):
    """
    map_rows: the enriched map stream from scripts/modern_map_stream_common.py
    ...
    """
    from map_feature_engine import apply_map_result, REQUIRED_MAP_STREAM_COLUMNS
