...
combination, so validation/validate_phase5c1.py requires the V3-arm fold metrics
...
from evaluation.validation.evaluate_series_feature_sets_v3 import (
...
def write_report(fold_df, agg_df, delta_df, delta_agg, diag_df, rf_importance_df, xgb_importance_df, group_dfs,
    ...
    md.append("**Regression parity.** V3-arm fold metrics here are required (by `validation/validate_phase5c1.py`) "
    ...
    (REPORTS / "phases" / "phase5c1_player_roster_cv_results.md").write_text("\n".join(md), encoding="utf-8")
