...
EVALUATION_ARTIFACTS = {
    "protocol": ROOT / "config" / "evaluation" / "phase8e_cologne_simulation_vs_reality_protocol.yaml",
    ...
    "report": ROOT / "reports" / "phases" / "phase8e_cologne_simulation_vs_reality.md",
}
...
EVALUATION_SCRIPTS = [
    ...
    "evaluation/cologne_2026/phase8e_metrics.py", "evaluation/cologne_2026/build_phase8e_summary.py",
    "evaluation/cologne_2026/build_phase8e_source_snapshot.py", "evaluation/cologne_2026/build_phase8e_metrics_detail.py",
    "evaluation/cologne_2026/phase8e_figures.py", "validation/validate_phase8e.py", "evaluation/cologne_2026/build_phase8e_receipt.py",
]
...
def run_validator():
    result = subprocess.run([sys.executable, "-m", "validation.validate_phase8e"],
                             cwd=str(ROOT), capture_output=True, text=True,
                             encoding="utf-8", env={"PYTHONIOENCODING": "utf-8", **__import__("os").environ})
    print(result.stdout[-2000:])
    if result.returncode != 0:
        raise RuntimeError("STOP: validation/validate_phase8e.py failed - refusing to write the receipt. "
...
def build():
    preflight_check()
    print("=== running validation/validate_phase8e.py before committing the receipt ===")
    run_validator()
